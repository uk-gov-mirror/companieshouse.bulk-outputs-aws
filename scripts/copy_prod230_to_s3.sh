# This script copies product 230 to an s3  bucket.  It can copy:
# the xml file or
# the b64 files or   
# both the xml file and the b64 files

this_progname="$(basename $BASH_SOURCE)"
this_product="prod230"
product_type="free"
...
# Repeat the validation of the 230 variant in copy_bulkoutputs_to_s3 in case
# this script is called directly
variant="${1}"
if [[ -z $variant || ! "$valid_prod230_variants" =~ "$variant" ]] ;
    then
...
do
    case $i in
        xml)
            prod230_file_pattern="Prod230_????*"$i"*"
            latest_file=`ls -tr $bulk_output_dir$prod230_file_pattern | tail -1`
            echo "Latest prod230 file is $latest_file"
