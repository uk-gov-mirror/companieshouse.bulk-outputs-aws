# This script copies product 213 variants to an Amazon S3 bucket.  

this_progname="$(basename $BASH_SOURCE)"
this_product="prod213"
product_type="free"
...
# Repeat the validation of the prod213 variant in copy_bulkoutputs_to_s3 in case
# this script is called directly

variant="${1}"
if [[ -z $variant || ! "$valid_prod213_variants" =~ "$variant" ]] ;
    then
...
prod213_file_pattern=""

latest_file=""

previous_file="$previous_files_dir/prod213_"${1}
...
case $variant in 
    ...
    llp)
       variant_list="llp" ;;
    all_variants)
       variant_list="all llp" ;;
esac

for i in $variant_list 
do
    prod213_file_pattern="Prod213_????_"$i"*"
    latest_file=`ls -tr $bulk_output_dir$prod213_file_pattern | tail -1`
    echo "Latest prod213 file is $latest_file"
    previous_file="$previous_files_dir/prod213_"$i
    echo "Previous prod213 file is $previous_file"
