# This script copies product 214 variants to an Amazon S3 bucket.  

this_progname="$(basename $BASH_SOURCE)"
this_product="prod214"
product_type="free"
...
# Repeat the validation of the 214 variant in copy_bulkoutputs_to_s3 in case
# this script is called directly


variant="${1}"
if [[ -z $variant || ! "$valid_prod214_variants" =~ "$variant" ]] ;
    then
...
do
    prod214_file_pattern="Prod214_????_*"$i"*"
    latest_file=`ls -tr $bulk_output_dir$prod214_file_pattern | tail -1`
    echo "Latest prod214 file is $latest_file"
    previous_file="$previous_files_dir/prod214."$i
    echo previous prod214 file is $previous_file
